...
async def _gather_report_metrics(
    ...
    offline_devices = (
        (await db.execute(select(Device).where(Device.status == "offline"))).scalars().all()
    )

    # Infrastructure Load Metrics
    top_cpu_devices = (
        (await db.execute(select(Device).order_by(Device.cpu_percent.desc().nullslast()).limit(5)))
        .scalars()
        .all()
    )

    high_memory_devices = (
        (await db.execute(select(Device).where(Device.memory_percent > 85.0).limit(5)))
        .scalars()
        .all()
    )

    avg_latency = (
        await db.execute(select(func.avg(Device.latency_ms)).where(Device.status == "online"))
    ).scalar_one_or_none() or 0.0

    # 2. Onboarding Workflow Summary (Filtered by dates if provided)
    onboard_stmt = select(func.count(OnboardedUser.id))
    if start_date:
        onboard_stmt = onboard_stmt.where(OnboardedUser.created_at >= start_date)
    if end_date:
        onboard_stmt = onboard_stmt.where(OnboardedUser.created_at <= end_date)
    ...
    if start_date:
        audit_stmt = audit_stmt.where(AuditLog.timestamp >= start_date)
    if end_date:
        audit_stmt = audit_stmt.where(AuditLog.timestamp <= end_date)
    ...
    async with db_factory() as db:
        try:
            ...
            await send_email_with_attachment(
                ...
                body=(
                    "Hello,\n\nHere is your weekly automated infrastructure "
