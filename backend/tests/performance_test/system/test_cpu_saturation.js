...
export const options = {
  scenarios: {
    scans: {executor: "constant-vus", vus: Number(__ENV.SCAN_VUS || 2),
      duration: __ENV.DURATION || "2m", exec: "scanWorkload"},
    reports: {executor: "constant-vus", vus: Number(__ENV.REPORT_VUS || 2),
  ...
  // saturation test - we want to OBSERVE degradation, not fail on it
  thresholds: {
...
export function scanWorkload(data) {
  const subnet = __ENV.SCAN_SUBNET || "10.0.0.0/30";
  const r = http.post(`${BASE_URL}/scan`,
    JSON.stringify(envJson("SCAN_PAYLOAD", {subnet})),
    {headers: jsonHeaders(data.token), tags: {name: "POST /scan [cpu]"}, timeout: "60s"});
  check(r, {"scan accepted": x => x.status === 202 || x.status === 200});
  if (r.status!== 202) console.log(`SCAN failed ${r.status} ${r.body}`);
  sleep(1);
}
...
export function reportWorkload(data) {
  const payload = envJson("REPORT_PAYLOAD", {
    ...
    start_date: __ENV.REPORT_START_DATE || "2026-08-01",
    end_date: __ENV.REPORT_END_DATE || "2026-08-29",
  });
  const r = http.post(`${BASE_URL}/reports/trigger`, JSON.stringify(payload),
    {headers: jsonHeaders(data.token), tags: {name: "POST /reports/trigger [cpu]"}, timeout: "60s"});
...
export function readWorkload(data) {
  const r = http.get(`${BASE_URL}/devices?page=1&page_size=50`,
    {headers: authHeaders(data.token), tags: {name: "GET /devices [cpu]"}, timeout: "10s"});
